 ...
 * - 5 locale を各言語の自称表記（localeScaffold.title を再利用）で表示。内部 locale ID は出さない。
  ...
  return (
    <nav aria-label={label} className="mt-6">
      <ul className="flex flex-wrap justify-center gap-x-4 gap-y-1">
        {locales.map((l) => {
          // 言語名は各言語の自称表記（対象 locale 辞書の localeScaffold.title を再利用）。
          const name = getDictionary(l).localeScaffold.title;
          if (l === locale) {
            return (
              <li key={l}>
                <span
